...
export class VisitorInviteService  {
    ...
    */
    async createInvite(
        ...
        if(requiresSeat) {
            const isSeatAvailable = await this.parkingService.isParkingAvailable(inviteDate);
            if(!isSeatAvailable) {
                throw new ParkingNotFound("Seat not available");
            }
        } 
        ...
        const info = await this.mailService.sendInvite(visitorEmail, userEmail, inviteID, idDocType, requiresSeat, inviteDate);
