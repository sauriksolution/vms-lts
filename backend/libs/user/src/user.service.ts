...
export class UserService{
    ...
    async getPendingVisitors() {
        return this.queryBus.execute(new GetUnAuthUsersQuery(-3));
    }
    ...
    async approveVisitor(email: string) {
        // Change visitor permission from -3 to 3 (approved visitor)
        const res = await this.commandBus.execute(new AuthorizeUserCommand(email));
        return res.modifiedCount > 0;
    }
    ...
    async deauthorizeUserAccount(email: string) {
        ...
        async addUserDirectly(email: string) {
        const res = await this.commandBus.execute(new AuthorizeUserCommand(email));        
        return res.modifiedCount > 0;
    }

    async createUserByAdmin(email: string, password: string, name: string, idNumber: string, idDocType: string, pinNumber: string, file: string, userType: string) {
        const dateString = (new Date()).toLocaleDateString();
        const typeCounts = await this.rewardService.getTypeCounts();
        const badgeCount = (await this.rewardService.getAllBadges()).length;
        let badges ="1";
        ...
        // Set permission based on user type
    ...
    async createVisitorRegistration(visitorData: {
        email: string;
        password: string;
        name: string;
        idNumber: string;
        ...
        // Create visitor with permission -3 (pending approval)
        // Store additional visitor data in the user record
        const visitorUser = {
            ...visitorData,
            permission: -3, // Pending visitor approval
            badges,
            sleepoverCount: (-1*typeCounts["sleepover"]),
            themeCount: (-1*typeCounts["theme"]),
            inviteCount: (-1*typeCounts["invite"]),
            curfewTime: (-100*typeCounts["curfew"]),
            dateJoined: dateString,
            pinNumber: '0000' // Default pin for visitors
        };
        
        return this.commandBus.execute(new CreateUserCommand(
            visitorUser.email, 
            visitorUser.password, 
            visitorUser.permission, 
            visitorUser.idNumber, 
            visitorUser.idDocType, 
            visitorUser.name, 
            visitorUser.badges, 
            visitorUser.sleepoverCount,
            visitorUser.themeCount,
            visitorUser.inviteCount,
            visitorUser.curfewTime, 
            visitorUser.dateJoined, 
            visitorUser.pinNumber, 
            visitorUser.file
        ));
    }
