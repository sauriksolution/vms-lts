...
def train(modelSavePath, numNeighbours, knnAlgo="ball_tree", verbose=False):
    ...
    if numNeighbours is None:
        numNeighbours = int(round(math.sqrt(len(encodings))))

    # Ensure at least 1 neighbor and not more than available samples
    numNeighbours = max(1, min(numNeighbours, len(encodings)))
