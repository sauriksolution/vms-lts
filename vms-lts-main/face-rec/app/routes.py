...
def initializeClassifier():
    global knnClassifier
    try:
        if os.path.exists("./model.plk"):
            print("Loading classifier from model.plk")
        ...
        print(f"Error loading classifier, training new one: {e}")
        knnClassifier = updateClassifier()

    if knnClassifier is None:
        print("Warning: No training data found, classifier not loaded")
...
def comparefaces():
    if request.method == "POST":
        ...
        if file and allowedFile(file.filename):
            if knnClassifier is None:
                return createResponse({"error": "Classifier not trained - no known faces"})
